...
import com.skysoul.accountremebercompose.model.repository.UserRepository
import com.skysoul.accountremebercompose.model.repository.resultbeans.LoginResult
import com.skysoul.accountremebercompose.config.StaticConfig
...
class UserRepositoryLocal : UserRepository, BaseRepository() {
...
//    }
    ...
    override suspend fun getUserTipByUsername(userName: String): SSResult<String> {
        return safeDBCall {
            userDao.getTipByUsername(userName).run {
    ...
    override suspend fun updateUserPassword4ViewAccount(
        userId: Int,
        password: String?,
        isUseLoginPassword: Boolean
    ): SSResult<Boolean> {
        return safeDBCall {
            userDao.getUserById(userId)?.run {
                this.isSamePassword = isUseLoginPassword
                if (password != null) {
    ...
    override suspend fun registerWith(
        ...
        return safeDBCall {
            userDao.insert(DMUser(0, userName, MDPasswordInfo(password), passwordTip = passwordTip))
                .run {
    ...
    override suspend fun checkPasswordWithType(
        ...
        userName: String,
        password: String
    ): SSResult<LoginResult> {

        return safeDBCall {
            // TODO: 查看用户名是否存在和密码是否正确，剩余次数等，都不应该直接获取用户对象，应该直接查相关字段进行判断
            ...
            if (dmUser == null) {
                ...
                if (leftTimes == 0) {
                    SSResult.Success(LoginResult(nextTryTime = lastWrong, errorType = -2))
                } else {
                    ...
                    if (password2Compare == MD5.getMD5(password)) {
                        ...
                        if (leftTimes == 0) {
                            ...
                            SSResult.Success(LoginResult(leftTimes = leftTimes, errorType = -3))
                    }
                }
            }
        }
    ...
    override suspend fun isUserNameExist(userName: String): SSResult<Boolean> {
        return safeDBCall {
